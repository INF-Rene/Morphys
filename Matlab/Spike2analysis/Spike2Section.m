classdef Spike2Section < Sharedpaths & Spike2Setupsettings
    
    %   Spike2Section: A Matlab class for loading in only a section of a
    %   Spike2 generated .mat file. Since these files are generally very
    %   large and contain lots of useless information, this class is used
    %   to extract data that only belongs to a section where a single
    %   protocol is applied.
    %   
    %   Notes:
    %   Most of this code is based on the Abffile Matlab class and
    %   belonging subclasses written by Thijs Verhoog
    %   ---------------------------------------------------------------------------------------------------------------------
...
%###################################################### PROPERTIES ##########################################################
    ...
    properties (Access = public)
        ...
        fileconversiondate  % date of conversion
        filesystemdate      % system date of abf file storage
        filename            % original file name
        filedirectory       % original storage directory of file
        filetype            % mat file generated by spike2 (maybe later also the original Spike2 swr file?)
        filesize            % size in bytes of original ABFfile
        fileduration        % total duration of recording
        
        sectionstartkey     % key that was used to define the starttime of the section
        sectionstartkeynr   % the occurence of the specified start key that was used as section start
        sectionendkey       % if empty, the sectionendtime is defined as sectiontimestart+sectionmaxduration
        sectionmaxduration  % duration of the full protocol. If this and endkey are empty, the script will predict the end by looking at the secondary channels
        sectiontimestart    % time in the file in which the section starts
        sectiontimeend      % time in the file in which the section ends
        sweeplength         % sweep length belonging to the used protocol. If empty the file will be treated as a single sweep with a lot of epochs
        savename            % name to use when saving this file
        
        nrofsweeps  = 0     % number of sweeps
...
%####################################################### METHODS ############################################################
    ...
    methods
        ...
        % ----------------------------------------- CLASS CONSTRUCTOR--------------------------------------------------------
        function obj = Spike2Section(path2file,setup, varargin) 
            obj = obj@Sharedpaths;
            obj = obj@Spike2Setupsettings(setup);
            ...
            elseif ~ischar(path2file)
                ...
                if isempty(ext), 
                    path2file = [path2file '.mat']; % add ".abf" if required
            ...
            % load
            path2file='G:\China_rene\20180521_test\ccstep test.mat'; %remove this line later
            datastruct = load(path2file);
            %
            ...
            obj.filetype       = 'Spike2 generated mat file';
            obj.filesize       = fileinfo.bytes;
            obj.filesystemdate = fileinfo.date; 
            
            % create a savename
            obj.savename = sprintf('Spike2_%s_%s.mat',obj.filename(1:end-4),obj.guid);

            % define section start & end
            obj.sectionstarttime=datastruct.(obj.keychannelname).times(datastruct.(obj.keychannelname).codes == unicode2native(obj.sectionstartkey));
            if isfield(obj,'sectionendkey') && ~isempty(obj.sectionendkey)
                obj.sectionstoptime=datastruct.(obj.keychannelname).times(datastruct.(obj.keychannelname).codes == unicode2native(obj.sectionendkey));
            elseif isfield(obj,'sectionmaxduration') && isempty(obj.sectionmaxduration)
                obj.sectionstoptime=obj.sectionstarttime+obj.sectionmaxduration;
            else
                %predict the end function
            end
            
            % obtain possible channels from Spike2Setupsettings
            obj.channels=ss.channels;
            
            %secoffset=chdata.(secch).start-chdata.(ch).start;

            % create timeseries
            ts=timeseries(datastruct.(obj.ch).values,[0:(datastruct.(ch).length-1)].*datastruct.(ch).interval);
            ts=ts.getsampleusingtime(starttime,stoptime);
            sects=timeseries(datastruct.(secch).values,[0:(datastruct.(secch).length-1)].*datastruct.(secch).interval+secoffset);
            sects=sects.getsampleusingtime(starttime,stoptime);    
            % get analogindata
            chs=fieldnames(chdata);
            chs(contains(obj.chs, obj.StringCh))=[]; %adds all channels except the stringchannel with the protocol information
            for i=1:numel(chs)
                if all(ismember({'length', 'interval'}, chdata.(chs{i})))
                    samples(i)=chdata.(chs{i}).length;
                    sampleints(i)=chdata.(chs{i}).interval;
                    starts(i)=chdata.(chs{i}).start;
                else
                    error('Missing channel information in spike2 generated mat file')
                end
            obj.fileduration  = duration(0,0,0,max(samples.sampleints)*1e3,'Format',obj.durationfmt);
            end
