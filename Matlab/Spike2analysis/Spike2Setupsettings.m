classdef Spike2Setupsettings < Sharedmethods
...
%###################################################### PROPERTIES ##########################################################    
    properties
        ...
        keychannel = 31;        % Channel with key input information. As far as I know always Ch31
        keydelay = 7;           % approximate delay of actually starting the protocol and registering the keystroke in Ch31. Might differ from computer to computer due to speed. On my computer the delay was usually 6.5-7.5 ms.        
    end
    
%####################################################### METHODS ############################################################
    methods
        % ----------------------------------------- CLASS CONSTRUCTOR -------------------------------------------------------
        function obj = Spike2Setupsettings(setuptemplate)
            ...
            if nargin == 0, return
            elseif nargin == 1,
                if isa(setuptemplate,'Spike2Setupsettings')
        ...
        function obj = addchannel(obj, varargin)
            ...
            if numel(varargin)==1 && isa(varargin{1},'Spike2Channel')
                chnnl2add = varargin{1};
            else
                chnnl2add = Spike2Channel(varargin{:});% make the channel
            end
        ...
        function obj = updatechannelstats(obj)
            ...
            for i = 1:numel(obj)
                ...
                if obj(i).nrofchannels == 0,
                    obj(i).channelnrs      = [];
                    obj(i).analoginputnames  = [];
                else
                    obj(i).channelnrs      = obj(i).getchannelnrs;
                    obj(i).analoginputnames  = obj(i).getanaloginputnrs;
                end
            end
        end
        ...
        function checkchannel(obj,chnnl)
            ...
            elseif ismember({chnnl.primary,chnnl.secondary},obj.analoginputnames), 
                                                                error('Analog input number already in use.')
            elseif isempty([chnnl.number,chnnl.primary,chnnl.secondary]); 
