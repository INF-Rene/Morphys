chdata=load('C:\Users\DBHeyer\Documents\PhD\China\Djai\Recordings\mats\180705_000.mat');
%settings
ch='Ch1';
secch='Ch2';
StringCh='Ch31';
CCStepkey='a';
stopkey='s';
sweeplength=4000; %length of each sweep in ms
...
secoffset=chdata.(secch).start-chdata.(ch).start;

% create timeseries
ts=timeseries(chdata.(ch).values,[0:(chdata.(ch).length-1)].*chdata.(ch).interval);
ts=ts.getsampleusingtime(starttime,stoptime);
sects=timeseries(chdata.(secch).values,[0:(chdata.(secch).length-1)].*chdata.(secch).interval+secoffset);
sects=sects.getsampleusingtime(starttime,stoptime);

%find epoch edges and holding values in secondary timeseries
baseline=sects.getsamples(1:1000).mean;    %warning: assumption that the first 1000 samples are always without current injection
...
keydelay=7; %approximate delay of actually starting the protocol and registering the keystroke in Ch31. May differ from computer to computer due to speed
...
edgeidx1=edgeidx*2.5;
edgeidx1(edgeidx1>max(sweepidx1))=[];
edgeidx1=sort([edgeidx1 ; sweepidx1]);

sweepidx = round([1 [1:numsweeps]*(sweeplength-1)/chdata.(secch).interval/1000]+keydelay/1000/chdata.(secch).interval)';
...
for i=1:numsweeps
    ...
    s2 = s2.adddata(get(sects.getsamples(sweepidx(i):sweepidx(i+1)-1), 'Data'),chdata.(secch).units);
    s2 = s2.addtime(1/chdata.(secch).interval);
