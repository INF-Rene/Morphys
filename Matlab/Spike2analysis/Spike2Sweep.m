classdef Spike2Sweep < Sharedmethods & Trace
...
%% ##################################################### METHODS ############################################################
    methods
        %% ---------------------------------------- CLASS CONSTRUCTOR -------------------------------------------------------
        function obj = Spike2Sweep(varargin)
        ...
        function obj = addepoch(obj,epochtab)
            ...
            switch lower(epochtab.typestr)
                case {'step','chirp','noisepp','noisespiking','truenoise'},
                            if isempty(obj.epochs), obj.epochs        = Spike2Epoch(epochtab);
                            else                    obj.epochs(end+1) = Spike2Epoch(epochtab);
                            end
                case {'ramp', 'train'},                 %RWS
                            if isempty(obj.epochs), obj.epochs        = Spike2Epoch(epochtab,obj.getstartendepochwave(obj.nrofepochs,'end'));
                            else                    obj.epochs(end+1) = Spike2Epoch(epochtab,obj.getstartendepochwave(obj.nrofepochs,'end'));
                            end
