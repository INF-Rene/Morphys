...
%use this part instead of redo-ing all analysis
    ...
    %if ccabf == 1 
      fprintf('Retrieving analysis parameters from CC-2 file %1.0f \n', index);
        %% Analyze
        ...
        for step = 1:length(sweep(1).epoch)
            if sweep(1).epoch(2).stepdiff ~= 0 && (sweep(1).epoch(2).stepdiff + sweep(1).epoch(2+1).stepdiff) == 0 && seconds(sweep(1).epoch(2).timespan) > 0.03
                break
            end
        end
        %for djai files has to go 
%         for j = 1:NrofSweeps
%             sweep(j).vmbase = sweep(j).epoch(2-1).steadystate ;
%             sweep(j).jitter = sweep(j).epoch(2-1).jitter ;
...
%                 %if sweep(j).epoch(2+1).ap(ap).start_time > (sum(second({sweep(1).epoch(1:2).timespan}))*1000)+7 && sweep(j).epoch(2+1).ap(ap).start_time < (sum(second({sweep(1).epoch(1:2).timespan}))*1000)+300
...
%      if strcmp({sweep(j).epoch.idxstr}, 'B')
%          sweep(j).epoch.typestr = 'step' ; 
%      else 
%          sweep(j).epoch.typestr =  'x' ; 
%      end
            sweep(j).vmbase = sweep(j).epoch(1).steadystate ;
            sweep(j).jitter = sweep(j).epoch(1).jitter ;
 ...
 Nrofabfs = length(abf) ;
        ...
        for j = 1:NrofSweeps           
            if sweep(j,1).currinj >= -100 && sweep(j,1).currinj < 0
                ...
                if sweep(j,1).epoch(2).tau < 100 && sweep(j,1).epoch(2).tau > 0 && sweep(j,1).epoch(2).gof > 0.95
                    taus(j,1) = sweep(j,1).epoch(2).tau ;
                else
                    taus(j,1) = NaN;
                end
            end       
        ...
        TrainCurr = sweep(frstspikeswp).epoch(2).stepdiff +50 ;
        for j = 1:NrofSweeps
            tmp(j) = abs(sweep(j).epoch(2).stepdiff - TrainCurr) ;
        end
       ...
       % changed this to 250 pA
        TrainCurr = sweep(frstspikeswp).epoch(2).amplitude +50 ;
        for j = 1:NrofSweeps
            step = find(strcmp({sweep(j).epoch.idxstr}, 'B'));
            tmp(j) = abs(sweep(j).epoch(2).amplitude - TrainCurr) ;
        end
        ...
        for TrainSweep = TrSwp:NrofSweeps  
           step = find(strcmp({sweep(TrainSweep).epoch.idxstr}, 'B'));
            if length(sweep(TrainSweep).ap) > 1
                TrSweepCrit=1;
                CurrAbvRheo = sweep(TrainSweep).epoch(2).amplitude - (TrainCurr-50) ;               
                break
               end
...
%             TrSweepCrit=1;
%             CurrAbvRheo = sweep(TrainSweep).epoch(2).stepdiff - (sweep(frstspikeswp).epoch(2).stepdiff) ;
%         else
%             TrSweepCrit=0;
%         end
        ...
        if TrSweepCrit==1
            ...
            OnsetTSFAP = NaN ;%sweep(TrainSweep).ap(1).thresh_time - (sum(second({sweep(TrainSweep).epoch(1:2-1).timespan}))*1000) ;
        else
            TSbasetothresh = NaN;
            TSpeaktoahp = NaN;
            AmpsTSthresh = NaN;
         ...
         TrainCurr2 = sweep(frstspikeswp).epoch(2).amplitude +150 ;
        for j = 1:NrofSweeps
            step = find(strcmp({sweep(j).epoch.idxstr}, 'B'));
            tmp(j) = abs(sweep(j).epoch(2).amplitude - TrainCurr2) ;
        end
        ...
        for TrainSweep2 = TrSwp2:NrofSweeps  
            ...
            if length(sweep(TrainSweep2).ap) > 1
                TrSweepCrit2=1;
                CurrAbvRheo2 = sweep(TrainSweep2).epoch(2).amplitude - (TrainCurr2-150) ;               
                break
               end
        ...
        Sag                = sweep(sagswp,1).epoch(2).sag / PkDeflect(sagswp,1) ;
        Sag2               = sweep(sagswp2,1).epoch(2).sag / PkDeflect(sagswp2,1) ;
        Sag3               = sweep(sagswp3,1).epoch(2).sag / PkDeflect(sagswp3,1) ;
...
%         end
       ...
       % Summary(index).PDur               = seconds(sweep(1).epoch(2).timespan)*1000 ;
        ...
        Summary(index).CurrAbvRheo1       = CurrAbvRheo ;
    ...
    %    Summary(index).curr_FrstAP        = sweep(frstspikeswp).ap(1).currinj ; 
        Summary(index).OnsetFrstAP        = NaN; %sweep(frstspikeswp).ap(1).thresh_time - (sum(seconds({sweep(frstspikeswp).epoch(1:2-1).timespan}))*1000) ; 
       % here I made it epoch(2) instead of epoch(4) EM 221005
...
%%
basedir = '/Users/elinemertens/Data/ephys/Summary/2023';  % specify path to analyzed abfBatch. File will be saved in this folder
savename = 'Summary_173_test' ;
save(fullfile(basedir, savename), 'Summary') ;

%%

 Summary_All = struct2table(Summary) ; 
 writetable(Summary_All, 'Summary_ayoub4.xlsx');
