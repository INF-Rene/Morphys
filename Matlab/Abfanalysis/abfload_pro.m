function [d,si,h]=abfload_pro(fn,varargin)
...
% convert names in structure to variables and re    ad value from header
for g=1:numOfParams
...
if h.fFileVersionNumber>=2
  ...
  % --- add bigString to h struct to obtain original directory and protocol filename in wrapper 
  % (added 11-08-2016, Thijs Verhoog)
  h.stringSection = char(BigString);
%   [~,abfname]=fileparts(fn);
%   assignin('base',sprintf('StringSection_%s',abfname),char(BigString)')
  ...
  % --- read in epoch information (current injection protocol in episoodic stimulation aquisition mode)
  if EpochPerDACSection.llNumEntries > 0
      for i = 1:EpochPerDACSection.llNumEntries
        h.EpochSec(i,1) = ReadSection(fid,EpochPerDACSection.uBlockIndex*BLOCKSIZE+EpochPerDACSection.uBytes*(i-1),EpochInfo);
      end
  end
  
  % --- store ADCsection as well...
  h.ADCSec = ADCsec';
...
switch h.nOperationMode
  case 1  
    disp(' --> Acquisition:   Event-driven variable-length mode');
    h.nOperationModeStr = 'Event-driven variable-length mode';
  ...
  case {2,4,5}
    if h.nOperationMode==2
      disp(' --> Acquisition:   Event-driven fixed-length mode');
      h.nOperationModeStr = 'Event-driven fixed-length mode';
    elseif h.nOperationMode==4
      disp(' --> Acquisition:   High-speed oscilloscope mode');
      h.nOperationModeStr = 'High-speed oscilloscope mode';
    else
      disp(' --> Acquisition:   Waveform fixed-length mode');
      h.nOperationModeStr = 'Waveform fixed-length mode';
    end
    ...
    % Following data points check changed by Thijs from error to warning. Once in a while this check fails even though
    % all seems fine. 
    h.datapointsok = 1;
    if rem(h.dataPts,h.nADCNumChannels)>0 || rem(h.dataPtsPerChan,h.lActualEpisodes)>0
      warning('number of data points not OK');
      h.datapointsok = 0;
    end
  ...
  case 3
    disp(' --> Acquisition:   Gap-free mode');
    h.nOperationModeStr = 'Gap-free mode';
  ...
  otherwise
    disp(' --> Acquisition:   Unknown recording mode -- returning empty matrix');
    h.nOperationModeStr = 'Unknown recording mode';
    d=[];
    h.si=[];
end
fclose(fid);
...
switch fileSig
 ...
 case 'ABF2'
   ...
   headPar={
     ...
     'FileGUID',40,'uint8',ones(1,16); % Thijs: doesn't get the fileGUID out in original form... needs some reshaping
