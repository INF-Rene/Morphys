...
function [] = parseStruct(s,docNode,curNode,pName)
    ...
    for i = 1:length(fnames)
        ...
        elseif (strcmp(curfield,'Text'))
            ...
            %Sub-element
            assignin('base','currs',s)
            assignin('base','curfield',curfield)
            if (isstruct(s.(curfield)))
                %single element
...
function [str,succes] = val2str(val)
    ...
    succes = true;
    str = ''; % adapted by Thijs Verhoog according to comments on http://nl.mathworks.com/matlabcentral/fileexchange/28639-struct2xml, on 23 feb 2017
    
    if (isempty(val))
        return; %bugfix from H. Gsenger
    elseif (ischar(val))
        %do nothing
    elseif (isnumeric(val)  ||  islogical(val)) % adapted by Thijs Verhoog according to comments on http://nl.mathworks.com/matlabcentral/fileexchange/28639-struct2xml, on 23 feb 2017
        val = num2str(val);
    else
        succes = false;
    end
