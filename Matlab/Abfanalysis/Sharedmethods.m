...
%###################################################### PROPERTIES ##########################################################
    ...
    properties (Hidden = true)      % default analysis settings
        ...
        apThreshDvdt        = 23;   % (mV/ms) dVdt threshold for determining AP threshold. Value of 10 mV/ms usually does well, but perhaps ~20 best for recordings with more high-frequency noise...    
        apThreshRapidity    = 40;   % (mV/ms) onset rapidity will be measured as slope of phase plane when it crosses this threshold.    
        apMinPeakDistance   = 2;    % (ms) refractory period; minimum time between peaks for peak detection.
        apMinPeakProminence = 20;   % (mV) The amount of mV the peak has to stand out from the minimum between the neighboring peaks and/or the beginning/end of sweep
        ...
        postAPtime          = 20;   % maximum time after AP to be taken into waveform
        ...
        onsetrapfitwin      = 6;    % number of samples (in effect microseconds) to include in the onset rapidity fit, before and after the crossing of the onset rapidity dvdt threshold (apThreshRapidity)
        ...
        dynamicresampling   = 0;    % set to 0 if you don't want any resampling. Set do 1 for performance increase.
