classdef Channel < Sharedmethods
...
%###################################################### PROPERTIES ########################################################## 
    properties
        ...
        dacnum              % number of the DAC channel. This is the number of the analog OUT bnc on the digidata that connects to the command bnc of this channel.
        primary             % number of the ADC channel receiving the primary output
        secondary           % number of the ADC channel receiving the secondary output
        scope               % number of the ADC channel receiving the scope output
        mode                % number of the ADC channel receiving the mode output (voltage/current clamp, rarely used)
        
        trigger             % number of a trigger channel. Not necessarily unique associated with this amplifier channel, but good to have the option, for instance, if one records the Master8 output triggering an EPSP in one of the Analogins.
        ...
        analogouts          % list of Analogouts (= Analogout class instances)
        analoginnrs         % numbers of analog input  channels (corresponding to numbers of digidata)
        analogoutnrs        % numbers of analog output channels (corresponding to numbers of digidata)
        nrofanalogins  = 0  % number of Analog Input channels recorded
        nrofanalogouts = 0  % number of Analog Output channels enabled  
    end
...
%####################################################### METHODS ############################################################
    methods
        ...
        function n = getanaloginputnrs(obj)
            ...
            if isscalar(obj)
                n = [obj.get('primary'),obj.get('secondary'),obj.get('scope'),obj.get('mode'),obj.get('trigger')];
            elseif isvector(obj)
        ...
        function obj = addin(obj,infostruct,channeldata)
            ...
            in2add = Analogin(infostruct,channeldata,obj.guid);
